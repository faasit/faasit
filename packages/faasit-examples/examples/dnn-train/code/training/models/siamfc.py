    ...
    def _initialize_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.bias.data.fill_(0)  # type: ignore
...
class TrackerSiamFC(Tracker):
    ...
    def init(self, image, box):
        ...
        # exemplar features
        exemplar_image = (
            torch.from_numpy(exemplar_image)
            .to(self.device)
            .permute([2, 0, 1])
            .unsqueeze(0)
            .float()
    ...
    def update(self, image):
        ...
        instance_images = (
            torch.from_numpy(instance_images)
            .to(self.device)
            .permute([0, 3, 1, 2])
            .float()
        )
        ...
        responses = np.stack(
            [
                cv2.resize(
                    t, (self.upscale_sz, self.upscale_sz), interpolation=cv2.INTER_CUBIC
                )
                for t in responses
            ],
            axis=0,
    ...
    def _crop_and_resize(self, image, center, size, out_size, pad_color):
        ...
        if npad > 0:
            image = cv2.copyMakeBorder(
                image, npad, npad, npad, npad, cv2.BORDER_CONSTANT, value=pad_color
            )
