...
def ProfileFollowers(frt : FaasitRuntime):
	...
	for single_request in review_request:
		# [req_id, request_type, username]
		req_id, request_type, username = single_request
		if request_type != Info.RequestType.Profile.value:
		...
		if user is None:
			response.append([req_id, 'Invalid username name in review update.'])
			continue

		success_msg += 1
		response.append([req_id, f"{username} has {len(user['followers'])} followers."])
...
profileFollowers = ProfileFollowers.export()